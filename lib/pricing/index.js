...
function roundPsychological(amount, currency) {
  ...
  // Use charm endings with 9 (e.g. .49/.99) and round up to avoid undercutting.
  const base = Math.floor(amount);
  const candidates = [base + 0.49, base + 0.99, base + 1.49];
  const next = candidates.find((candidate) => candidate >= amount);
  return Number((next || base + 1.49).toFixed(2));
}
...
export function calculatePrice({ basePriceINR, countryCode, currencyOverride }) {
  ...
  const tier = selectedCurrency === "INR" ? "india" : "international";
