...
export async function createCouponFromAdminPayload(payload, adminEmail) {
  ...
  const duplicateSnapshot = adminDb
    ? await adminDb
        ...
        .limit(10)
        .get()
    : await getDocs(query(collection(db, "coupons"), where("codeUpper", "==", normalized.code), limit(10)));
  ...
  const created = adminDb
    ? await adminDb.collection("coupons").add(nextData)
    : await addDoc(collection(db, "coupons"), nextData);
