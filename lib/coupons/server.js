...
export function normalizeCouponDocument(raw, id) {
  ...
  return {
    ...
    userEmail: normalizeCouponEmail(coupon.userEmail),
...
function sanitizeCouponPayload(input) {
  ...
  const userEmail = normalizeCouponEmail(input?.userEmail);
...
export async function validateCouponForCheckout({
  ...
  const normalizedEmail = normalizeCouponEmail(email);
  if (coupon.userEmail && normalizedEmail !== coupon.userEmail) {
...
export async function listCheckoutVisibleCoupons({
  ...
  const coupons = snapshot.docs
    ...
    .filter((coupon) => !coupon.userEmail || coupon.userEmail === normalizedEmail)
    .slice(0, 120);
...
export async function consumeCouponUsage({
  ...
  try {
    const result = await runTransaction(db, async (transaction) => {
      ...
      if (coupon.userEmail && coupon.userEmail !== normalizedEmail) {
