...
export async function getCouponByCode(code) {
  ...
  if (adminDb) {
    const snapshotByCodeUpper = await adminDb
      .collection("coupons")
      .where("codeUpper", "==", normalizedCode)
      .limit(1)
      .get();

    if (!snapshotByCodeUpper.empty) {
      const row = snapshotByCodeUpper.docs[0];
      return normalizeCouponDocument(row.data(), row.id);
    }

    const snapshotByCode = await adminDb
      .collection("coupons")
    ...
    if (!snapshotByCode.empty) {
      const row = snapshotByCode.docs[0];
      return normalizeCouponDocument(row.data(), row.id);
    }
  }

  const snapshotByCodeUpper = await getDocs(
  ...
  if (snapshotByCode.empty) return null;
  const row = snapshotByCode.docs[0];
  return normalizeCouponDocument(row.data(), row.id);
}
...
export async function getCouponById(couponId) {
  ...
  const adminDb = getAdminDb();
  if (adminDb) {
    const snapshot = await adminDb.collection("coupons").doc(normalizedId).get();
    if (snapshot.exists) {
...
export async function getCouponUserUsageCount(couponId, email) {
  ...
  if (adminDb) {
    const snapshot = await adminDb
      ...
      .where("couponId", "==", normalizedCouponId)
      .limit(500)
      .get();
...
async function addPurchaseCouponUsageStats({
  adminDb,
  couponId,
  email,
  resetMs,
  ...
  const applyPurchaseDoc = (raw = {}) => {
    const purchasedAtIso = toIsoDate(raw.purchasedAt);
    const usedAtMs = new Date(purchasedAtIso || 0).getTime();
    if (resetMs > 0 && (!Number.isFinite(usedAtMs) || usedAtMs < resetMs)) return 0;

    const paymentKey = String(raw.paymentId || "").trim();
    const orderKey = String(raw.orderId || raw.paymentId || "").trim();
    const uniqueKey = paymentKey || orderKey;
    if (uniqueKey && (paymentKeys.has(uniqueKey) || orderKeys.has(uniqueKey))) return 0;

    if (paymentKey) paymentKeys.add(paymentKey);
    if (orderKey) orderKeys.add(orderKey);
    return Math.max(1, Number(raw.quantity || 1));
  };
  ...
  if (adminDb) {
    ...
    return snapshot.docs.reduce((sum, docItem) => sum + applyPurchaseDoc(docItem.data() || {}), 0);
  }
...
export async function getCouponUserUsageStats(couponId, email) {
  ...
  if (adminDb) {
    ...
    snapshot.docs.forEach((docItem) => {
      ...
      const paymentKey = String(data.paymentId || "").trim();
      if (paymentKey) paymentKeys.add(paymentKey);
      const orderKey = String(data.orderId || data.paymentId || "").trim();
      if (orderKey) orderKeys.add(orderKey);
    });

    itemCount += await addPurchaseCouponUsageStats({
      adminDb,
    ...
    return {
      usageCount: paymentKeys.size || orderKeys.size,
...
export async function listCheckoutVisibleCoupons({
  ...
  const snapshot = adminDb
    ? await adminDb.collection("coupons").limit(300).get()
    : await getDocs(query(collection(db, "coupons"), limit(300)));
...
export async function createCouponFromAdminPayload(payload, adminEmail) {
  ...
  const duplicateSnapshot = adminDb
    ? await adminDb
      .collection("coupons")
      .where("codeUpper", "==", normalized.code)
      .limit(10)
      .get()
    : await getDocs(
  ...
  if (!duplicateSnapshot.empty) {
    ...
    if (recreateTarget?.id) {
      ...
      if (adminDb) {
        await adminDb.collection("coupons").doc(recreateTarget.id).update(nextData);
      } else {
        await updateDoc(doc(db, "coupons", recreateTarget.id), nextData);
      }
  ...
  const createdId = adminDb
    ? (await adminDb.collection("coupons").add(createData)).id
    : (await addDoc(collection(db, "coupons"), createData)).id;

  const fresh = await getCouponById(createdId);
  ...
  if (adminDb) {
    try {
      const result = await adminDb.runTransaction(async (transaction) => {
        const usageRef = adminDb.collection("coupon_usages").doc(usageDocId);
        const couponRef = adminDb.collection("coupons").doc(normalizedCouponId);
      ...
      return { ok: false, reason: "admin_transaction_failed", error: String(error?.message || error) };
    }
  }
